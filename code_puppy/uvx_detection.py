"""Detect if the CLI was launched via uvx on Windows.

This module provides utilities to detect the launch method,
specifically to handle signal differences when running via uvx on Windows.

On Windows, when launched via uvx, Ctrl+C (SIGINT) gets captured
...
def is_launched_via_uvx() -> bool:
    """Detect if the CLI was launched via uvx.
