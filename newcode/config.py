...
from newcode.session_storage import save_session


def _get_xdg_dir(env_var: str, fallback: str) -> str:
    """
    Get directory for newcode files, defaulting to ~/.newcode.

    XDG paths are only used when the corresponding environment variable
    is explicitly set by the user. Otherwise, we use the legacy ~/.newcode
    ...
    Returns:
        Path to the directory for newcode files
    """
    # Use XDG directory ONLY if environment variable is explicitly set
    xdg_base = os.getenv(env_var)
    if xdg_base:
        return os.path.join(xdg_base, "newcode")

    # Default to legacy ~/.newcode for all file types
    return os.path.join(os.path.expanduser("~"), ".newcode")
...
def load_mcp_server_configs():
    """
    Loads the MCP server configurations from XDG_CONFIG_HOME/newcode/mcp_servers.json.
    Returns a dict mapping names to their URL or config dict.
    If file does not exist, returns an empty dict.
    """
    from newcode.messaging.message_queue import emit_error
...
def get_project_agents_directory() -> Optional[str]:
    ...
    Looks for a .newcode/agents/ directory in the current working directory.
    ...
    """
    project_agents_dir = os.path.join(os.getcwd(), ".newcode", "agents")
    ...
    if not command_history_exists:
        try:
            ...
            old_history_file = os.path.join(
                os.path.expanduser("~"), ".newcode_history.txt"
            )
