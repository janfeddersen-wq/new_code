...
from newcode.command_line.attachments import parse_prompt_attachments
...
from newcode.terminal_utils import (
...
async def main():
    ...
    initialize_command_history_file()
    from newcode.messaging import emit_error, emit_system_message
    ...
    try:
        from newcode.uvx_detection import should_use_alternate_cancel_key
    ...
    if args.model:
        from newcode.config import _validate_model_exists
...
async def interactive_mode(message_renderer, initial_command: str = None) -> None:
    ...
    if initial_command:
        ...
        try:
            ...
            try:
                from newcode.tools.command_runner import is_awaiting_user_input
            ...
            if response is not None:
                ...
                from newcode.messaging import get_message_bus
                from newcode.messaging.messages import AgentResponseMessage
    ...
    try:
        ...
        try:
            ...
            from newcode.messaging import emit_error, emit_warning
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                try:
                    from newcode.terminal_utils import ensure_ctrl_c_disabled
            ...
            # Handle Ctrl+C (or CancelledError from prompt_toolkit's
            # prompt_async when the asyncio task is cancelled) - continue loop
        ...
        if task.strip():
            ...
            try:
                ...
                from newcode.messaging.queue_console import get_queue_console

                get_queue_console().print_exception()

            # Auto-save session if enabled (moved outside the try block to avoid being swallowed)
            from newcode.config import auto_save_session_if_enabled
            ...
            while is_wiggum_active():
                ...
                from newcode.messaging import emit_system_message, emit_warning
...
async def run_prompt_with_attachments(
    ...
    *,
    ...
    from newcode.agents.event_stream_handler import set_streaming_console
