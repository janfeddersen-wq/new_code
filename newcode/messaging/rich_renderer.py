...
from newcode.config import get_subagent_verbose
...
class RichConsoleRenderer:
        ...
        def render_dir_tree(dir_path: str, depth: int = 0) -> None:
            ...
            if dir_path == root_key:
                ...
                for f in sorted(files, key=lambda x: x.path):
                    ...
                    self._console.print(f"{bar}{indent}[green]{name}[/green]{size_str}")
            else:
    ...
    def _render_file_content(self, msg: FileContentMessage) -> None:
        ...
        self._console.print(
            f"[{color}]│[/{color}]   [bold cyan]{msg.path}[/bold cyan]{line_info}"
        )
    ...
    def _render_diff(self, msg: DiffMessage) -> None:
        ...
        from newcode.config import get_show_diffs
    ...
    def _render_skill_list(self, msg: SkillListMessage) -> None:
        ...
        if not msg.skills:
            ...
            self._console.print(
                f"{bar}[dim]  Install skills in ~/.newcode/skills/[/dim]"
            )
            return
