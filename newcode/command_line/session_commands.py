...
from newcode.config import CONTEXTS_DIR
from newcode.session_storage import list_sessions, load_session, save_session
...
def handle_compact_command(command: str) -> bool:
    ...
    from newcode.config import get_compaction_strategy, get_protected_token_count
...
def handle_load_context_command(command: str) -> bool:
    ...
    from newcode.agents.agent_manager import get_current_agent
    from newcode.config import rotate_autosave_id
    from newcode.messaging import emit_error, emit_info, emit_success, emit_warning
