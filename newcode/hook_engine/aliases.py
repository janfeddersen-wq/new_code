"""
Tool name alias registry — maps each AI provider's tool names to newcode's
...
Each provider block defines a dict[str, str] mapping:
    "<Provider tool name>" -> "<newcode internal tool name>"
...
CLAUDE_CODE_ALIASES: Dict[str, str] = {
    ...
    # NOTE: the tools below have no direct newcode equivalent yet.
...
GEMINI_ALIASES: Dict[str, str] = {
    # Add Gemini → newcode tool mappings here
}
...
CODEX_ALIASES: Dict[str, str] = {
    # Add Codex → newcode tool mappings here
}
...
SWARM_ALIASES: Dict[str, str] = {
    # Add Swarm → newcode tool mappings here
}
...
def resolve_internal_name(provider_tool_name: str) -> Optional[str]:
    """
    Return the newcode internal tool name for a given provider tool name,
