...
This plugin adds OAuth authentication for Claude Code, automatically importing available models into your configuration.
...
- **Token Management**: Secure storage of OAuth tokens in the application config directory
...
After authentication, models will be added to `extra_models.json` in the application data directory:
...
- **Token Storage**: Tokens are saved to `claude_code_oauth.json` in the application data directory with `0o600` permissions
...
- Ensure the browser completed the redirect back to the application (no pop-up blockers)
...
- **Plugin Registration**: Custom command handlers wired into the application
