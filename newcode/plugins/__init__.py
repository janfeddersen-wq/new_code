...
USER_PLUGINS_DIR = Path.home() / ".newcode" / "plugins"
...
def _load_builtin_plugins(plugins_dir: Path) -> list[str]:
    ...
    from newcode.config import get_safety_permission_level
    ...
    for item in plugins_dir.iterdir():
        if item.is_dir() and not item.name.startswith("_"):
            ...
            if callbacks_file.exists():
                ...
                try:
                    module_name = f"newcode.plugins.{plugin_name}.register_callbacks"
...
def _load_user_plugins(user_plugins_dir: Path) -> list[str]:
    """Load user plugins from ~/.newcode/plugins/.
...
def load_plugin_callbacks() -> dict[str, list[str]]:
    ...
    Loads plugins from:
    1. Built-in plugins in the newcode/plugins/ directory
    2. User plugins in ~/.newcode/plugins/
