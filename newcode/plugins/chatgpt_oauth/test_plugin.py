...
from newcode.plugins.chatgpt_oauth import config, utils
...
def test_config_paths():
    ...
    # XDG paths use "newcode" (without dot) in ~/.local/share or ~/.config
    assert "newcode" in str(token_path)

    config_dir = config.get_config_dir()
    # Default is ~/.newcode; XDG paths only used when XDG env vars are set
    assert config_dir.name in ("newcode", ".newcode")
...
@patch("newcode.plugins.chatgpt_oauth.utils.requests.post")
...
@patch("newcode.plugins.chatgpt_oauth.utils.requests.get")
