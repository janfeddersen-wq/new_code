...
class TestSchedulerListTasks:
    def test_with_tasks(self):
        ...
        task = _make_task()
        with (
            patch("newcode.scheduler.load_tasks", return_value=[task]),
    ...
    def test_with_tasks_various_statuses(self):
        ...
        with (
            patch("newcode.scheduler.load_tasks", return_value=tasks),
...
class TestSchedulerDeleteTask:
    def test_delete_found(self):
        ...
        task = _make_task()
        with (
            patch("newcode.scheduler.get_task", return_value=task),
...
class TestSchedulerDaemonStatus:
    def test_running(self):
        ...
        with (
            patch("newcode.scheduler.load_tasks", return_value=[_make_task()]),
            patch("newcode.scheduler.daemon.get_daemon_pid", return_value=123),
            patch("newcode.tools.scheduler_tools.emit_info"),
        ):
            result = fn()
