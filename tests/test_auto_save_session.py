...
from newcode.session_storage import SessionMetadata


@pytest.fixture
def mock_config_paths(monkeypatch):
    mock_home = "/mock_home"
    mock_config_dir = os.path.join(mock_home, ".newcode")
...
class TestAutoSaveSessionFunctionality:
    @patch("newcode.config.get_auto_save_session")
    ...
    @patch("newcode.config.save_session")
    @patch("newcode.config.get_current_autosave_session_name")
    @patch("newcode.config.datetime.datetime")
    @patch("newcode.config.get_auto_save_session")
...
class TestFinalizeAutoSaveSession:
    @patch("newcode.config.rotate_autosave_id", return_value="fresh_id")
    @patch("newcode.config.auto_save_session_if_enabled", return_value=True)
    ...
    @patch("newcode.config.rotate_autosave_id", return_value="fresh_id")
    @patch("newcode.config.auto_save_session_if_enabled", return_value=False)
