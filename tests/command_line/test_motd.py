...
class TestHasSeenMotd:
    ...
    def test_returns_false_when_file_does_not_exist(self):
        ...
        from newcode.command_line.motd import has_seen_motd

        with patch("newcode.command_line.motd.os.path.exists", return_value=False):
    ...
    def test_returns_true_when_version_in_file(self):
        ...
        with patch("newcode.command_line.motd.os.path.exists", return_value=True):
...
class TestMarkMotdSeen:
    ...
    def test_creates_directory_if_not_exists(self):
        ...
        from newcode.command_line.motd import mark_motd_seen

        with patch("newcode.command_line.motd.os.makedirs") as mock_makedirs:
            with patch("newcode.command_line.motd.os.path.exists", return_value=False):
    ...
    def test_appends_version_when_file_empty(self):
        ...
        with patch("newcode.command_line.motd.os.makedirs"):
            with patch("newcode.command_line.motd.os.path.exists", return_value=False):
...
class TestPrintMotd:
    ...
    def test_prints_motd_when_not_seen(self):
        ...
        with patch("newcode.command_line.motd.has_seen_motd", return_value=False):
            with patch("newcode.command_line.motd.emit_info") as mock_emit:
                with patch("newcode.command_line.motd.mark_motd_seen") as mock_mark:
                    result = print_motd()
    ...
    def test_does_not_print_when_already_seen(self):
        ...
        with patch("newcode.command_line.motd.has_seen_motd", return_value=True):
            with patch("newcode.command_line.motd.emit_info") as mock_emit:
                with patch("newcode.command_line.motd.mark_motd_seen") as mock_mark:
                    result = print_motd()
    ...
    def test_emits_markdown_content(self):
        ...
        with patch("newcode.command_line.motd.has_seen_motd", return_value=False):
            with patch("newcode.command_line.motd.emit_info") as mock_emit:
                with patch("newcode.command_line.motd.mark_motd_seen"):
    ...
    def test_marks_current_version_as_seen(self):
        ...
        from newcode.command_line.motd import MOTD_VERSION, print_motd

        with patch("newcode.command_line.motd.has_seen_motd", return_value=False):
            with patch("newcode.command_line.motd.emit_info"):
                with patch("newcode.command_line.motd.mark_motd_seen") as mock_mark:
                    print_motd()
...
class TestMotdConstants:
    ...
    def test_motd_version_is_string(self):
        ...
        from newcode.command_line.motd import MOTD_VERSION
    ...
    def test_motd_message_is_string(self):
        ...
        from newcode.command_line.motd import MOTD_MESSAGE
    ...
    def test_motd_track_file_ends_with_motd_txt(self):
        ...
        from newcode.command_line.motd import MOTD_TRACK_FILE

        # Just verify the filename - CONFIG_DIR is dynamic during tests
        assert MOTD_TRACK_FILE.endswith("motd.txt")
        assert ".newcode" in MOTD_TRACK_FILE or "newcode" in MOTD_TRACK_FILE
