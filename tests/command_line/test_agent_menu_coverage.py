...
class TestSanitizeDisplayText:
    ...
    def test_strips_emojis(self):
        result = _sanitize_display_text("Code Agent \U0001f436")
...
class TestGetPinnedModel:
    @patch(
        "newcode.command_line.agent_menu.get_agent_pinned_model",
        return_value="gpt-4",
    )
    def test_builtin_pinned(self, mock_pin):
        assert _get_pinned_model("agent1") == "gpt-4"

    @patch("newcode.command_line.agent_menu.get_agent_pinned_model", return_value=None)
    ...
    def test_exception_in_builtin(self, mock_pin):
        with patch(
            "newcode.agents.json_agent.discover_json_agents", side_effect=Exception
        ):
            assert _get_pinned_model("agent1") is None
...
class TestSelectPinnedModel:
    @pytest.mark.asyncio
    @patch("newcode.command_line.agent_menu.arrow_select_async")
    @patch(
        "newcode.command_line.agent_menu.load_model_names", return_value=["m1", "m2"]
    )
    @patch("newcode.command_line.agent_menu.get_agent_pinned_model", return_value=None)
    ...
    @pytest.mark.asyncio
    @patch(
        "newcode.command_line.agent_menu.arrow_select_async",
        side_effect=KeyboardInterrupt,
    )
    @patch("newcode.command_line.agent_menu.load_model_names", return_value=["m1"])
    @patch("newcode.command_line.agent_menu.get_agent_pinned_model", return_value=None)
    ...
    @pytest.mark.asyncio
    @patch(
        "newcode.command_line.agent_menu.load_model_names",
    ...
    @pytest.mark.asyncio
    @patch("newcode.command_line.agent_menu.load_model_names", return_value=[])
    @patch("newcode.command_line.agent_menu.get_agent_pinned_model", return_value=None)
...
class TestReloadAgentIfCurrent:
    @patch("newcode.command_line.agent_menu.get_current_agent")
    ...
    @patch("newcode.command_line.agent_menu.emit_info")
    @patch("newcode.command_line.agent_menu.get_current_agent")
...
class TestApplyPinnedModel:
    ...
    @patch(
        "newcode.command_line.agent_menu.set_agent_pinned_model",
        side_effect=Exception("fail"),
    )
...
class TestGetAgentEntries:
    @patch(
        "newcode.command_line.agent_menu.get_agent_descriptions",
        return_value={"a": "Desc A", "b": "Desc B"},
    )
    @patch(
        "newcode.command_line.agent_menu.get_available_agents",
    ...
    @patch("newcode.command_line.agent_menu.get_agent_descriptions", return_value={})
    @patch(
        "newcode.command_line.agent_menu.get_available_agents",
        return_value={"a": "A"},
    )
...
class TestRenderMenuPanel:
    @patch("newcode.command_line.agent_menu._get_pinned_model", return_value=None)
    ...
    @patch("newcode.command_line.agent_menu._get_pinned_model", return_value="gpt-4")
...
class TestInteractiveAgentPicker:
    ...
    @patch("newcode.command_line.agent_menu.get_current_agent")
    @patch("newcode.command_line.agent_menu.get_agent_descriptions", return_value={})
    @patch("newcode.command_line.agent_menu.get_available_agents", return_value={})
    ...
    @patch("newcode.command_line.agent_menu.emit_info")
    @patch("newcode.command_line.agent_menu.Application")
    @patch("sys.stdout")
