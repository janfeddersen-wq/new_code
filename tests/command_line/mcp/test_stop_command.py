...
class TestStopCommand:
    def test_no_args_shows_usage(self, stop_cmd):
        with patch("newcode.command_line.mcp.stop_command.emit_info") as mock_emit:
            stop_cmd.execute([], group_id="g1")
            assert mock_emit.called

    def test_generates_group_id(self, stop_cmd):
        with patch("newcode.command_line.mcp.stop_command.emit_info"):
            stop_cmd.execute([])
    ...
    def test_server_not_found(self, stop_cmd):
        with (
            ...
            patch("newcode.command_line.mcp.stop_command.suggest_similar_servers"),
            patch("newcode.command_line.mcp.stop_command.emit_info") as mock_emit,
        ):
    ...
    def test_stop_success(self, stop_cmd):
        with (
            ...
            patch("newcode.command_line.mcp.stop_command.emit_info") as mock_emit,
            patch(
                "newcode.command_line.mcp.stop_command.get_current_agent"
            ) as mock_agent,
        ):
    ...
    def test_stop_success_agent_reload_fails(self, stop_cmd):
        with (
            ...
            patch("newcode.command_line.mcp.stop_command.emit_info"),
            patch(
                "newcode.command_line.mcp.stop_command.get_current_agent",
    ...
    def test_outer_exception(self, stop_cmd):
        with (
            ...
            patch("newcode.command_line.mcp.stop_command.emit_error") as mock_err,
        ):
            stop_cmd.execute(["myserver"], group_id="g1")
