"""Tests for newcode/command_line/mcp/wizard_utils.py"""
...
class TestInteractiveServerSelection:
    @patch("newcode.command_line.mcp.wizard_utils.emit_prompt")
    @patch("newcode.command_line.mcp.wizard_utils.emit_info")
    def test_select_server_valid(self, mock_info, mock_prompt):
        from newcode.command_line.mcp.wizard_utils import (
...
class TestInteractiveGetServerName:
    ...
    def test_custom_name(self, mock_prompt):
        from newcode.command_line.mcp.wizard_utils import interactive_get_server_name
...
class TestInteractiveConfigureServer:
    @patch("newcode.command_line.mcp.wizard_utils.install_server_from_catalog")
    @patch("newcode.command_line.mcp.wizard_utils.emit_prompt")
    @patch("newcode.command_line.mcp.wizard_utils.emit_info")
    def test_new_server_confirmed(self, mock_info, mock_prompt, mock_install):
        ...
        with patch(
            "newcode.command_line.mcp.utils.find_server_id_by_name",
            return_value=None,
        ):
    ...
    @patch("newcode.command_line.mcp.wizard_utils.emit_error")
    @patch("newcode.command_line.mcp.wizard_utils.emit_prompt")
    @patch("newcode.command_line.mcp.wizard_utils.emit_info")
...
class TestInstallServerFromCatalog:
    ...
    def test_successful_install(self, mock_info, tmp_path):
        from newcode.command_line.mcp.wizard_utils import install_server_from_catalog
...
class TestRunInteractiveInstallWizard:
    @patch(
        "newcode.command_line.mcp.wizard_utils.interactive_configure_server",
        return_value=True,
    )
    @patch(
        "newcode.command_line.mcp.wizard_utils.interactive_get_server_name",
        return_value="my-srv",
    )
    @patch("newcode.command_line.mcp.wizard_utils.interactive_server_selection")
    @patch("newcode.command_line.mcp.wizard_utils.emit_info")
    ...
    @patch(
        "newcode.command_line.mcp.wizard_utils.interactive_server_selection",
        return_value=None,
    )
    @patch("newcode.command_line.mcp.wizard_utils.emit_info")
