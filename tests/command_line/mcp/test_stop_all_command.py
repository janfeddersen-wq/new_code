...
def stop_all_cmd():
    with patch("newcode.command_line.mcp.base.get_mcp_manager") as mock_mgr:
        ...
        from newcode.command_line.mcp.stop_all_command import StopAllCommand
...
class TestStopAllCommand:
    def test_no_servers(self, stop_all_cmd):
        ...
        with patch("newcode.command_line.mcp.stop_all_command.emit_info") as mock_emit:
    ...
    def test_generates_group_id(self, stop_all_cmd):
        stop_all_cmd.manager.list_servers.return_value = []
        with patch("newcode.command_line.mcp.stop_all_command.emit_info"):
    ...
    def test_stop_success(self, stop_all_cmd):
        ...
        with (
            patch("newcode.command_line.mcp.stop_all_command.emit_info"),
            patch(
                "newcode.command_line.mcp.stop_all_command.get_current_agent"
            ) as mock_agent,
        ):
            stop_all_cmd.execute([], group_id="g1")
    ...
    def test_async_loop_exists(self, stop_all_cmd):
        ...
        with (
            patch("newcode.command_line.mcp.stop_all_command.emit_info"),
            patch("newcode.command_line.mcp.stop_all_command.get_current_agent"),
            patch("time.sleep"),
