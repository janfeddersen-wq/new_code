...
def edit_cmd():
    with patch("newcode.command_line.mcp.base.get_mcp_manager") as mock_mgr:
        ...
        from newcode.command_line.mcp.edit_command import EditCommand
...
class TestEditCommand:
    def test_no_args_shows_usage(self, edit_cmd):
        with patch("newcode.command_line.mcp.edit_command.emit_info") as mock_emit:
            edit_cmd.execute([], group_id="g1")
            assert mock_emit.call_count == 2

    def test_generates_group_id(self, edit_cmd):
        with patch("newcode.command_line.mcp.edit_command.emit_info"):
            edit_cmd.execute([])  # no group_id

    def test_config_file_not_exists(self, edit_cmd):
        with (
            patch("os.path.exists", return_value=False),
            patch("newcode.command_line.mcp.edit_command.emit_error") as mock_err,
            patch("newcode.command_line.mcp.edit_command.emit_info"),
        ):
    ...
    def test_server_not_found(self, edit_cmd):
        ...
        with (
            ...
            patch("newcode.command_line.mcp.edit_command.emit_error") as mock_err,
            patch("newcode.command_line.mcp.edit_command.emit_warning"),
            patch("newcode.command_line.mcp.edit_command.emit_info"),
        ):
            edit_cmd.execute(["myserver"], group_id="g1")
    ...
    def test_server_not_found_shows_available(self, edit_cmd):
        ...
        with (
            patch("os.path.exists", return_value=True),
            patch("builtins.open", mock_open(read_data=json.dumps(data))),
            patch("newcode.command_line.mcp.edit_command.emit_error"),
            patch("newcode.command_line.mcp.edit_command.emit_warning") as mock_warn,
            patch("newcode.command_line.mcp.edit_command.emit_info") as mock_info,
        ):
    ...
    def test_successful_edit(self, edit_cmd):
        ...
        with (
            ...
            patch("builtins.open", mock_open(read_data=json.dumps(data))),
            patch(
                "newcode.command_line.mcp.edit_command.run_custom_server_form",
                return_value=True,
            ) as mock_form,
            patch(
                "newcode.command_line.mcp.edit_command.reload_mcp_servers",
                create=True,
            ),
        ):
    ...
    def test_successful_edit_no_reload(self, edit_cmd):
        ...
        with (
            ...
            saved = sys.modules.get("newcode.agent")
            sys.modules["newcode.agent"] = None  # will cause ImportError
            try:
                edit_cmd.execute(["myserver"], group_id="g1")
            finally:
                if saved is not None:
                    sys.modules["newcode.agent"] = saved
                else:
                    sys.modules.pop("newcode.agent", None)
