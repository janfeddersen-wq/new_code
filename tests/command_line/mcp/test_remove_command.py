...
def remove_cmd():
    with patch("newcode.command_line.mcp.base.get_mcp_manager") as mock_mgr:
        ...
        from newcode.command_line.mcp.remove_command import RemoveCommand
...
class TestRemoveCommand:
    def test_no_args_shows_usage(self, remove_cmd):
        with patch("newcode.command_line.mcp.remove_command.emit_info") as mock_emit:
            remove_cmd.execute([], group_id="g1")
            assert "Usage" in str(mock_emit.call_args)

    def test_generates_group_id(self, remove_cmd):
        with patch("newcode.command_line.mcp.remove_command.emit_info"):
            remove_cmd.execute([])

    def test_server_not_found(self, remove_cmd):
        with (
            patch(
                "newcode.command_line.mcp.remove_command.find_server_id_by_name",
                return_value=None,
            ),
            patch("newcode.command_line.mcp.remove_command.suggest_similar_servers"),
            patch("newcode.command_line.mcp.remove_command.emit_info") as mock_emit,
        ):
    ...
    def test_remove_success_no_config_file(self, remove_cmd):
        with (
            ...
            patch("newcode.command_line.mcp.remove_command.emit_info"),
    ...
    def test_outer_exception(self, remove_cmd):
        with (
            ...
            patch("newcode.command_line.mcp.remove_command.emit_error") as mock_err,
        ):
            remove_cmd.execute(["myserver"], group_id="g1")
