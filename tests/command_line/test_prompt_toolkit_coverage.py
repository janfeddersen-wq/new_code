...
class TestSafeFileHistory:
    def test_store_string_success(self, tmp_path):
        from newcode.command_line.prompt_toolkit_completion import SafeFileHistory
...
class TestSetCompleter:
    ...
    def test_just_trigger_suggests_space(self):
        from newcode.command_line.prompt_toolkit_completion import SetCompleter
    ...
    def test_shows_config_keys(self):
        ...
        with (
            patch(
                "newcode.command_line.prompt_toolkit_completion.get_config_keys",
                return_value=["debug", "model", "yolo_mode", "puppy_token"],
            ),
            patch(
                "newcode.command_line.prompt_toolkit_completion.get_value",
                return_value="false",
            ),
        ):
            completions = list(c.get_completions(self._make_doc("/set "), None))
...
class TestCDCompleter:
    ...
    def test_no_trigger(self):
        from newcode.command_line.prompt_toolkit_completion import CDCompleter
    ...
    def test_completes_directories(self, tmp_path):
        ...
        c = CDCompleter()
        with patch(
            "newcode.command_line.prompt_toolkit_completion.list_directory",
            return_value=(["mydir"], []),
        ):
...
class TestAgentCompleter:
    ...
    def test_no_trigger(self):
        from newcode.command_line.prompt_toolkit_completion import AgentCompleter
    ...
    def test_shows_agents(self):
        ...
        with (
            ...
            patch(
                "newcode.command_line.pin_command_completion._get_agent_display_meta",
...
class TestSlashCompleter:
    ...
    def test_no_slash(self):
        from newcode.command_line.prompt_toolkit_completion import SlashCompleter
    ...
    def test_just_slash(self):
        ...
        with (
            patch(
                "newcode.command_line.prompt_toolkit_completion.get_unique_commands",
                return_value=[mock_cmd],
            ),
            patch("newcode.plugins.load_plugin_callbacks"),
            patch("newcode.callbacks.on_custom_command_help", return_value=[]),
        ):
    ...
    def test_custom_commands_list_format(self):
        ...
        with (
            ...
            patch("newcode.plugins.load_plugin_callbacks"),
            patch(
                "newcode.callbacks.on_custom_command_help",
    ...
    def test_custom_commands_exception(self):
        ...
        with (
            ...
            patch(
                "newcode.plugins.load_plugin_callbacks",
                side_effect=Exception("fail"),
            ),
        ):
            # Should not raise
...
class TestGetInputWithCombinedCompletion:
    ...
    async def test_basic(self):
        ...
        with (
            ...
            patch(
                "newcode.command_line.prompt_toolkit_completion.PromptSession"
            ) as mock_cls,
        ):
...
class TestKeyBindings:
    ...
    def test_bracketed_paste_empty_with_image(self, captured_bindings):
        ...
        with (
            patch(
                "newcode.command_line.prompt_toolkit_completion.has_image_in_clipboard",
                return_value=True,
            ),
            patch(
                "newcode.command_line.prompt_toolkit_completion.capture_clipboard_image_to_pending",
...
class TestAttachmentPlaceholderProcessorPaths:
    ...
    def test_no_detections(self):
        ...
        with patch(
            "newcode.command_line.prompt_toolkit_completion._detect_path_tokens",
            return_value=([], []),
        ):
            result = proc.apply_transformation(mock_ti)
            assert result is not None
    ...
    def test_with_image_detection(self):
        ...
        with (
            ...
            patch(
                "newcode.command_line.prompt_toolkit_completion._tokenise",
...
class TestClipboardCoverageAdditions:
    ...
    def test_safe_open_image_pil_unavailable(self):
        from newcode.command_line.clipboard import _safe_open_image

        with patch("newcode.command_line.clipboard.PIL_AVAILABLE", False):
    ...
    def test_safe_open_image_decompression_bomb(self):
        ...
        with (
            patch("newcode.command_line.clipboard.PIL_AVAILABLE", True),
            patch("newcode.command_line.clipboard.Image") as mock_img_mod,
        ):
    ...
    def test_get_linux_clipboard_image_no_tool(self):
        from newcode.command_line.clipboard import _get_linux_clipboard_image

        with patch(
            "newcode.command_line.clipboard._check_linux_clipboard_tool",
            return_value=None,
        ):
    ...
    def test_get_linux_clipboard_image_wl_paste(self):
        ...
        with (
            ...
            patch(
                "newcode.command_line.clipboard.subprocess.run",
    ...
    def test_has_image_linux_no_tool(self):
        from newcode.command_line.clipboard import has_image_in_clipboard

        with (
            patch("newcode.command_line.clipboard.sys.platform", "linux"),
            patch(
                "newcode.command_line.clipboard._check_linux_clipboard_tool",
                return_value=None,
            ),
    ...
    def test_get_clipboard_image_linux_large_no_pil(self):
        from newcode.command_line.clipboard import get_clipboard_image
        ...
        with (
            patch("newcode.command_line.clipboard.sys.platform", "linux"),
            patch(
                "newcode.command_line.clipboard._get_linux_clipboard_image",
                return_value=large_bytes,
            ),
            patch("newcode.command_line.clipboard.PIL_AVAILABLE", False),
        ):
    ...
    def test_get_clipboard_image_linux_large_with_pil(self):
        ...
        with (
            ...
            patch("newcode.command_line.clipboard.PIL_AVAILABLE", True),
            patch(
                "newcode.command_line.clipboard._safe_open_image",
                return_value=mock_img,
            ),
            patch(
                "newcode.command_line.clipboard._resize_image_if_needed",
                return_value=mock_img,
            ),
        ):
    ...
    def test_get_clipboard_image_image_mode_conversion(self):
        ...
        with (
            patch("newcode.command_line.clipboard.sys.platform", "darwin"),
            patch("newcode.command_line.clipboard.PIL_AVAILABLE", True),
            patch("newcode.command_line.clipboard.ImageGrab") as mock_grab,
            patch("newcode.command_line.clipboard.Image") as mock_img_mod,
        ):
    ...
    def test_get_pending_images_no_binary_content(self):
        from newcode.command_line.clipboard import ClipboardAttachmentManager

        mgr = ClipboardAttachmentManager()
        mgr.add_image(b"fake")

        with patch("newcode.command_line.clipboard.BINARY_CONTENT_AVAILABLE", False):
            assert mgr.get_pending_images() == []

    def test_check_linux_clipboard_tool_timeout(self):
        import subprocess

        from newcode.command_line.clipboard import _check_linux_clipboard_tool
    ...
    def test_resize_max_dimension_width(self):
        ...
        from newcode.command_line.clipboard import _resize_image_if_needed
        ...
        with patch("newcode.command_line.clipboard.Image") as mock_mod:
