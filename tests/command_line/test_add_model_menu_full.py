...
def _make_provider(name="TestProvider", env=None, provider_id="test"):
    from newcode.models_dev_parser import ProviderInfo
...
def _make_model(name="test-model", tool_call=True):
    from newcode.models_dev_parser import ModelInfo
...
class TestPromptForCredentials:
    ...
    @patch("newcode.command_line.add_model_menu.emit_info")
    ...
    @patch("newcode.command_line.add_model_menu.safe_input", return_value="")
    ...
    @patch("newcode.command_line.add_model_menu.emit_info")
    @patch("newcode.command_line.add_model_menu.set_config_value")
    @patch(
        "newcode.command_line.add_model_menu.safe_input", return_value="my-key-value"
    )
    ...
    @patch("newcode.command_line.add_model_menu.emit_warning")
    @patch(
        "newcode.command_line.add_model_menu.safe_input",
...
class TestAddModelMenuRun:
    ...
    @patch("newcode.command_line.add_model_menu.emit_info")
    @patch("newcode.command_line.add_model_menu.emit_error")
    @patch("newcode.command_line.add_model_menu.set_awaiting_user_input")
    @patch("newcode.command_line.add_model_menu.sys")
    @patch("newcode.command_line.add_model_menu.Application")
    def test_run_unsupported(self, mock_app, mock_sys, mock_await, mock_err, mock_info):
        from newcode.command_line.add_model_menu import UNSUPPORTED_PROVIDERS
    ...
    @patch("newcode.command_line.add_model_menu.emit_info")
    @patch("newcode.command_line.add_model_menu.emit_warning")
    @patch("newcode.command_line.add_model_menu.safe_input", return_value="n")
...
class TestInteractiveModelPicker:
    @patch("newcode.command_line.add_model_menu.AddModelMenu")
...
class TestPostTuiFlows:
    ...
    @patch("newcode.command_line.add_model_menu.emit_info")
    @patch("newcode.command_line.add_model_menu.safe_input")
