...
class TestHandleApiCommand:
    def test_status_not_running(self, tmp_path, monkeypatch):
        from newcode.command_line.core_commands import handle_api_command

        monkeypatch.setattr("newcode.config.STATE_DIR", str(tmp_path))
        with patch("newcode.messaging.emit_info"):
    ...
    def test_stop_running(self, tmp_path, monkeypatch):
        ...
        with patch("newcode.messaging.emit_success"), patch("os.kill"):
