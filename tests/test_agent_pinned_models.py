...
def mock_config_paths(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_config_dir = os.path.join(tmp_dir, ".newcode")
        tmp_config_file = os.path.join(tmp_config_dir, "puppy.cfg")
        monkeypatch.setattr("newcode.config.CONFIG_DIR", tmp_config_dir)
        monkeypatch.setattr("newcode.config.CONFIG_FILE", tmp_config_file)
